...
- **持久记忆**   Just md file. ~~五层渐进式记忆架构（Soul → Core → Work → Project → Daily），跨会话保留认知~~
...
这些文件通过 CLI 参数在启动时注入 Claude Code，通过 bare 模式忽略系统CC的配置（`--append-system-prompt-file`、`--plugin-dir`、`--add-dir`），详见 [`docs/claude-code-startup.md`](docs/claude-code-startup.md)。
...
| [Codex](https://github.com/openai/codex) | ✅ 已支持后台任务 |
...
| `/shell-restart` `/restart-shell` | 全局 | 重启整个 Shell 进程（launchd 自动拉起，代码更新生效）🔒 |
| `/status` | 当前会话 | 查看 Director 状态摘要 |
| `/help` | 全局 | 显示可用命令列表 |

`/start-with-codex` / `/start-with-claude` 用于群级启动模式切换：在一个小群里发送这类命令，Shell 会为该群创建或重建一个使用指定后端的 Director，后续群消息继续走该 Director，直到该群会话被回收或再次切换。
...
```
IM 消息 → TS Shell → MessageQueue → SessionBridge (主)
              │                         │
              │                    ┌────┴────┐
              ...
              │                    └─────────┘
              │
              │  群聊 → DirectorPool ─→ SessionBridge (群1)
              │                    └──→ SessionBridge (群2)
              │
...
```
SessionBridge          — 会话编排：FLUSH / 队列 / bootstrap / 事件
  └─ SessionAdapter    — 协议适配：Claude stream-json / Codex turn-based
      └─ Runtime       — 进程管理：Claude daemon (FIFO) / Codex per-turn spawn
```

SessionBridge 不关心底层是 Claude 还是 Codex，通过 adapter 接口统一调度。
...
1. Director 通过 MCP 工具调用 `create_task`，指定角色、prompt、描述，必要时可显式指定 `agent`
2. Shell 按角色默认 agent 或请求参数中的 `agent`，为任务 spawn 对应后端进程
...
如果你想让某个小群直接由 Codex 驱动，而不改全局配置，可以在群里发送 `/start-with-codex`。如果要切回默认的 Claude 体验，就发送 `/start-with-claude`。这些切换只影响当前群；其他群和私聊仍按默认配置运行。

注意：当前 Codex 小群 Director 是**会话常驻、进程按 turn 短驻**的 transport。也就是说，群会话和 session 会持续存在，但底层不是像 Claude 那样挂着一个常驻 daemon 进程；每轮消息都会通过 `codex exec` + `resume session` 续上历史。

Pool 有容量上限（默认 5 个），满时 LRU 淘汰最久未活跃的 Director。空闲超 30 分钟自动回收。

## Claude Code 启动细节

Shell 将 Claude Code CLI 作为子进程通过 FIFO named pipe 双向通信，完整的 CLI 参数链、stream-json 协议和会话恢复机制见 [`docs/claude-code-startup.md`](docs/claude-code-startup.md)。
...
│   ├── messaging.ts           # MessagingClient 接口定义
│   ├── messaging-router.ts    # 多渠道路由器（messageId → client）
│   ├── session-bridge.ts      # 会话桥：FLUSH / 队列 / bootstrap / 多后端调度
│   ├── director-session-adapter/  # 会话协议适配层
│   │   ├── claude.ts            # Claude stream-json 双向协议
│   │   └── codex.ts             # Codex turn-based 协议
│   ├── director-runtime/      # 进程生命周期层
│   │   ├── claude.ts            # Claude daemon（FIFO named pipe）
│   │   └── codex.ts             # Codex 按 turn spawn
│   ├── claude-process.ts      # Claude CLI 进程 spawn / PID / FIFO 管理
│   ├── director-pool.ts       # DirectorPool 多 Director 实例管理
│   ├── queue.ts               # 消息队列（correlation ID + cancel + 可选持久化）
│   ├── console.ts             # Web 控制台 + API + WebSocket 广播
│   ├── log-parser.ts          # Director 日志解析（对话重建 / session 列表）
│   ├── task-runner.ts         # 后台任务进程管理
│   ├── task-store.ts          # 任务 / Cron SQLite 存储
│   ├── task-mcp-server.ts     # MCP Server（Director 的任务工具）
│   ├── scheduler.ts           # Cron 调度器
│   ├── persona-process.ts     # Persona 实例 spawn（子角色通用）
│   ├── config.ts              # 配置加载 + agent provider 解析
│   ├── logger.ts              # 日志分级
│   └── public/                # Web 控制台前端
│       ├── index.html
│       ├── css/style.css
