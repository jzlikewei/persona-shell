...
3. **添加权限**（权限管理 → API 权限）：
   ...
   | `im:message.group_msg` | 获取群组中所有消息（敏感权限） | 群聊免@接收消息（必需，需单独申请） |
...
**解决方法**：飞书开放平台 → 你的应用 → 事件与回调 → 事件配置 → `im.message.receive_v1` → 将消息接收模式改为「**接收群中所有消息**」。此外还需在权限管理中申请敏感权限 `im:message.group_msg`（获取群组中所有消息），否则即使选了接收模式也不会推送非@消息。申请后需重新发布应用版本。
