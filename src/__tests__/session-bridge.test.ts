...
describe('SessionBridge', () => {
  ...
  test('switchAgent checkpoints, rebuilds adapter, bootstraps, and persists agent state', async () => {
    ...
    expect(initialAdapter.sent[0]).toContain('workspaces/test-bridge-Test Group.md');
    ...
    expect(switchedAdapter.sent[0]).toContain('workspaces/test-bridge-Test Group.md');
    expect(switchedAdapter.sent[0]).toContain('恢复这个会话的上下文');
    expect(existsSync('/tmp/persona-test/workspaces/test-bridge-Test Group.md')).toBe(true);
